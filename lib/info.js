 ...
 */
const generator = require('youtube-po-token-generator')
exports.getBasicInfo = async(id, options) => {
  ...
  if (!options.poToken) {
    console.log('Specify poToken for stable and fast operation. See README for details.');
    console.info('Automatically generates poToken, but stable operation cannot be guaranteed.');
    const { poToken, visitorData } = await generator.generate();
...
const fetchIosJsonPlayer = async(videoId, options) => {
  const payload = {
    ...
    serviceIntegrityDimensions: await generator.generate(),
    contentCheckOk: true,
    racyCheckOk: true,
    context: {
      client: {
  ...
  const opts = {
    requestOptions: {
      ...
      headers: {
        ...
        } (iPhone; CPU iPhone OS ${IOS_USER_AGENT_VERSION} like Mac OS X; en_US)`,
